...
| `promote_component` | **yes** (the *spec*, not the cluster), when `execute=true` | Pins one environment to the images another environment's latest deployed revision runs, and returns the resulting diff. Writes the spec store; deploys nothing. |
