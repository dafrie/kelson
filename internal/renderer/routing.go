...
func gatewayMissingError(app *model.ResolvedComponent, prov provenance, profile clusterprofile.ClusterProfile) Error {
	...
	remediation := "install a Gateway API implementation (`kelson install envoy-gateway` installs Envoy Gateway, the default candidate) and re-detect the cluster profile, or remove the domains from this component"
