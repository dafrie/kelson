...
- **Not a replacement for Flux.** Flux does the reconciling, always and only ([ADR-0028](adr/0028-delivery-spine.md)) — composing with an existing installation is the design point, and a cluster with none gets flux-aio installed with kelson itself: every Flux controller in one pod, sized for a small cluster ([ADR-0030](adr/0030-flux-aio-install.md)). Installing kelson installs its delivery substrate; a spec that silently never reconciles is not an outcome the product may produce. There is no second delivery path and no adapter seam; a second reconciler would be a new decision, argued on evidence.
