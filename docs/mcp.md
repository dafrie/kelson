...
| `set_secret` | **yes** (the *cluster*), when `execute=true` | Writes the Kubernetes Secret a spec's `{secret: <name>, key: <key>}` reference points at. Merges: keys it is not given are preserved. Also removes keys, with `remove_keys` instead of `values`: a key that is not in the Secret is refused rather than ignored, and removing the last key leaves an empty Secret — no tool deletes a Secret. Values travel in only — the answer reports names and keys, and no kelson API returns a secret value. |
