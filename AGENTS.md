...
| Path | Plane | Notes |
|---|---|---|
| `internal/model/` | authoring | Project / Component / Environment types, validation, `schema/*.json` generated by `internal/schemagen` |
| `internal/renderer/` | rendering | **pure**: `(spec, ClusterProfile) → manifests`. `Manifest` is an ordered `*yaml.Node`, never a Go map |
| `internal/delivery/` | delivery | the render → OCI artifact → Flux spine (ADR-0028) plus `statemachine/`; `direct/` and `git/` are being deleted in the rebuild (#223), `eject/` and `argocd/` are gone (ADR-0028, ADR-0012) |
