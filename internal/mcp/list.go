...
const listComponentsDescription = `List every project kelson has stored and the live state of each of its environments: the delivery phase, the deployed revision, how many workloads are healthy, and the cause when one is not.

READ-ONLY. Changes nothing.

The phase and revision come from the environment's own status and read "-" only when this server was started without the seam that reports it (a build-time absence, not a per-environment one) or the environment has not been deployed yet — either way the cause column says why. Workload health is independent of both and is what the "unhappy environment" judgement below rests on.
