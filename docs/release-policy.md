...
The kelson spec — `Project` and `Environment` in `kelson.dev/v1alpha1` — is a
versioned input to the renderer, not a private schema, and since
[ADR-0027](adr/0027-crd-native-control-plane.md) it is also a Kubernetes API
other people's tooling can read. It is what users, agents and the UI all author
...
The control-plane CRDs *are* the Project/Environment spec, and they share the
`kelson.dev/v1alpha1` API version — the CRD schemas are generated by
`internal/schemagen` from the same structs that produce `schema/*.json`, so
there is one answer to "what is a valid kelson spec"
([ADR-0027](adr/0027-crd-native-control-plane.md) decision 4). A breaking change to the schema *that
...
> A kelson spec that rendered successfully against `kelson vX.Y.Z` will
