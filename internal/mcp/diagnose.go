...
Read the WHY section first, and weigh each cause by its confidence rather than by its position. "high" means a controller named the reason — a kubelet waiting reason, a scheduler message, an external-secrets condition, an API-server rejection — or that two independent signals agreed; act on it. "medium" means one signal only, or a heuristic the server itself calls one, and its sentence says which half is a correlation; verify before acting. "low" is an audit-mode policy finding that vetoed nothing. The section also names the RECENT CHANGE: the revision now serving, when it started serving, the one before it, and what moved between their recorded images — read from the environment's own status (ADR-0028), the same record the deployment history above reads. What it still cannot do is a per-container or environment-variable diff, or name the revision that introduced a specific cause (a cause's "introduced by"): both need the recorded manifests of a revision, which this server does not fetch, and the section says so rather than a partial answer being mistaken for a complete one.
