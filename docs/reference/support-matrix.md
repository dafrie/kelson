...
| `flux` | `2.0.0` | — | refuse with a reason | the GitOps delivery flow: the GitRepository and Kustomization kelson commits against, and the chart source a HelmRelease fetches from |
...
| `argo-cd` | `2.9.0` | — | refuse with a reason | the Argo CD delivery path. No Argo adapter ships today (issue #138, ADR-0012), so nothing degrades yet — the floor is kept because the delivery seam stays pluggable |
...
- **`flux` 2.0.0** — flux v2's GitRepository/Kustomization API is what kelson commits against in GitOps mode.
