...
**Naming both Flux rows is refused.** Each installs a complete set of Flux controllers, so a cluster
given both would run two reconcilers over the same CRDs — `kelson install flux-aio flux` says so and
installs neither. Installing one now and the other later is fine: detection reports the Flux that is
already there and kelson adopts it.

> **Transition ([#226](https://github.com/dafrie/kelson/issues/226)).** The `flux-aio` row, its render
> script (`hack/flux-aio-render.sh`, `make flux-aio`) and the release-time verification step have landed.
> The rendered snapshot itself is generated by that script and committed — a kelson built from a tree
> where it has not been generated **refuses** `flux-aio`, naming the missing file and pointing at
> `kelson install flux`, and `--all-missing` on such a build offers full Flux instead of nothing. The CRD
> catalog row is still R3 work.
...
**Where the bytes live, and how to re-derive them.** The snapshot is
`internal/delivery/install/rendered/flux-aio.yaml`, compiled into the kelson binary by `//go:embed` and
verified against `Component.SHA256` before a byte of it is applied — so a tampered binary refuses on
your machine, not only in CI. Regenerate it with `make flux-aio`, which prints the pin block to paste
into `internal/delivery/install/pins.go`; `hack/flux-aio-render.sh --check` is the form the release
workflow runs, and it fails on any difference. A drift test in plain `go test ./...` fails if the
committed bytes and the pinned digest disagree, which is what makes "cannot be hand-edited" a property
rather than a promise.

**A build without the snapshot refuses, loudly.** The render needs network access to ghcr.io, so a tree
where `make flux-aio` has not run legitimately carries no snapshot. Such a build does not quietly skip
the row: `kelson install flux-aio` refuses, naming the missing file and pointing at `kelson install
flux`, and `--all-missing` stops preferring flux-aio and offers full Flux instead — because a cluster
left with no reconciler at all is a worse answer than six Deployments. The release workflow's `--check`
step means a released kelson can never be in that state.
