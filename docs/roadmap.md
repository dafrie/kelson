...
| Phase | Scope | Exit |
|---|---|---|
| **R1 · The spine** ([#224](https://github.com/dafrie/kelson/issues/224)) | `api/kelson/v1alpha1`, CRD generation out of `internal/schemagen`, `cmd/kelson-controller` on controller-runtime, the OCI publisher, the `OCIRepository` + `Kustomization` pair, status and history mirror. Deletes `internal/delivery/direct`, `git`, `rollback`, the adapter seam and the ConfigMap spec/history stores. **Status:** the six steps are built — publisher extracted to `internal/artifact` and shared with previews, spec hash and artifact ref, SSA'd Flux pair with `wait: true`, phase readback, rollback annotation, bounded history, the delivery error taxonomy and the `kelson.dev/environment` finalizer. The chart's controller RBAC has landed too: `patch` on `environments` for the finalizer (plus the kubebuilder `environments/finalizers` spelling, which a CRD does not serve on its own) on the cluster-scoped ClusterRole, a namespaced Role in the flux namespace for CRUD on `OCIRepository`/`Kustomization` (delete included, for the finalizer teardown), leader election on by default now that the controller writes rather than only reads (`deploy/chart/kelson`). Deliberately still no workload-reading RBAC — R1 delegates health to Flux's own `wait: true` and tracks the finer-grained readback separately ([#240](https://github.com/dafrie/kelson/issues/240)). The exit gate is now written as a test: `TestDeliverySpine` (`test/e2e/spine_test.go`, provisioned by `hack/e2e/spine.sh`, run by `.github/workflows/e2e.yml`) applies a Project and an Environment as custom resources and asserts the artifact, its provenance annotations, the `OCIRepository`/`Kustomization` pair, the live workload, a second revision, the rollback reverting the deployed bytes, and the resume — see [docs/e2e.md](e2e.md). **The exit gate is green**: run 62 of the e2e workflow passed `TestDeliverySpine` in 71s on kind — all nine steps, rollback byte-reversion and the inert resume included. Writing the test already paid for itself twice: it caught an inert rollback re-arming (`EnvironmentReconciler` discarded the generation `rollbackFor` computes for it), and its first CI run caught the chart granting only `environments/finalizers` — a subresource a CRD does not serve — so the finalizer patch was forbidden and `.status` was never written even though the artifact and the Flux pair were both healthy. Both are fixed with regression tests, in `internal/controller` and `deploy/chart` respectively | **a spec deployed end to end on a kind cluster** — apply a Project and an Environment, artifact published, Flux reconciles, `Environment.status` reaches Healthy |
| **R2 · Façade and verbs** ([#225](https://github.com/dafrie/kelson/issues/225)) | `kelson-server` over CRs with SSA and `resourceVersion` concurrency; the agent and audit stores to `internal/controlstore`; deploy, history, rollback (the annotation), promote (the CR patch) reshaped in the CLI, the UI and MCP. **Status:** the server façade is reshaped — `DeployService.{Deploy,Status,Rollback,History,Promote}` over the CRs, `SpecService` over SSA. `kelson deploy`/`status` (workload half)/`rollback`/`promote`/`history` are ConnectRPC clients of it again, addressing a spec inline or by stored project name (`--project`), with `--server`/`--password`/`--token` matching `kelson-mcp`'s. The MCP tools and the web UI are re-pointed too — deploy/rollback/promote descriptions match what the façade really does, and the UI renders the new history/rollback semantics honestly. The one gap R2 left has closed: `RenderService.Diff(from_revision)` pulls the revision's immutable artifact back out of the registry and diffs the recorded manifests ([#247](https://github.com/dafrie/kelson/issues/247), `internal/artifact`'s `Pull`), and the rollback preview reuses it rather than carrying `rollback/preview-unavailable` | every verb works over the new spine with the wire surface unchanged |
| **R3 · Install path** ([#226](https://github.com/dafrie/kelson/issues/226)) | the flux-aio catalog row and its release-time render, kelson's own CRDs as a catalog entry, chart and RBAC for the controller, previews re-verified against the shared publisher. **Status:** the release-hook half landed with [#227](https://github.com/dafrie/kelson/issues/227) (two-Kustomization `dependsOn`, [ADR-0037](adr/0037-release-hooks-on-the-spine.md)) and previews were re-verified against the OCI-artifact path earlier. The flux-aio machinery has landed too: `hack/flux-aio-render.sh` (`make flux-aio`) renders the module with a checksum-pinned timoni binary against a digest-pinned OCI module, writes `internal/delivery/install/rendered/flux-aio.yaml` with per-object provenance, and `.github/workflows/release.yml` re-renders and fails the release on any difference; the `flux-aio` pins row serves those bytes through the ordinary decode/provenance/uninstall path, and the sweep now offers it in preference to flux-operator on a Flux-less cluster, declining the operator by name and refusing a request that asks for both ([ADR-0030](adr/0030-flux-aio-install.md) decisions 1–4). **Remaining:** the rendered snapshot itself has not been generated — it needs one `make flux-aio` from a machine that can pull the module's OCI blobs, then the printed pin block committed alongside it; until then a build refuses `flux-aio` (naming the missing file, offering `kelson install flux`) and `--all-missing` falls back to full Flux, and the release `--check` fails. Then kelson's own CRDs as a catalog entry, and chart and RBAC for the controller | a cluster with nothing gets a working kelson in one install path |
