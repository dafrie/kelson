...
| `controller.pushSecret` | `--registry-config` | An existing `kubernetes.io/dockerconfigjson` Secret. Mounted into the pod (its `.dockerconfigjson` key projected onto `/etc/kelson/registry/config.json`) rather than read from the API, because the controller reads a file — the same one a CI `docker login` writes. Empty is an anonymous push. It needs pull scope as well as push: the controller lists the registry's tags to reach a revision older than the bounded history in `Environment.status` ([#241](https://github.com/dafrie/kelson/issues/241)). |
