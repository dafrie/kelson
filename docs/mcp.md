...
| `rollback` | **yes**, when `execute=true` | Previews what a rollback cannot revert (unrecoverable findings flagged; today that is always the one `rollback/preview-unavailable` finding, since revisions are immutable OCI artifacts and the server does not fetch two of them to diff); applies it on request. |
